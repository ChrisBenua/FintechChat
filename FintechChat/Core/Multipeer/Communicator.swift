...
class MultipeerCommunicator: NSObject, Communicator {
    ...
    var lastState: [String: MCSessionState] = [:]
...
extension MultipeerCommunicator: MCSessionDelegate {
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        ...
        self.lastState[peerID.displayName] = state
        
        if state == .connected {
            Logger.log("ConnectedPeer " + peerID.displayName)
        }
        if state == .notConnected {
            Logger.log("DisconnectedPeer " + peerID.displayName)
        }
        onDisconnectDelegate?.userDidDisconnected(stateDict: self.lastState)
